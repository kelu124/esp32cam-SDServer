...
void handleFileUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    uploadFile = fs.open(upload.filename.c_str(), FILE_WRITE);
...
void handleCreate() {
  ...
  if (path.indexOf('.') > 0) {
    File file = fs.open((char *)path.c_str(), FILE_WRITE);
    if (file) {
      file.write(0);
      file.close();
    }
  } else {
    fs.mkdir((char *)path.c_str());
  }
  returnOK();
}
...
server.on("/", HTTP_GET, printDirectory);
  ...
  if (SD_MMC.begin()) { 
    ...
    if (cardType != CARD_NONE) {
      DBG_OUTPUT_PORT.println("Carte SD Initialisee.");
      hasSD = true;
    }
  }
